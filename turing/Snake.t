...
%to choose mode
    process startmusic
    loop
    Music.PlayFile ("Main Menu.mp3")
    end loop
    end startmusic
    
    
function start_up : string
    setscreen ("graphics:1074, 603;nobuttonbar")
    drawfillbox (0, 0, 1074, 603, 7)
    ...
    gamecover := Pic.FileNew ("new cover.jpg")
    Pic.Draw (gamecover, 0, 0, picMerge)
    View.Update
    ...
    Font.Draw ("Try to block the other snake! ",100, 500, snake_font,white) 
    Font.Draw ("Get the fruits to make your snake longer!",100, 400, snake_font, white)
    Font.Draw ("Use arrows as Player 1!",100, 300, snake_font, white)
    Font.Draw ( "Use 'WASD' as player 2!", 100, 200, snake_font, white) 
    Font.Draw ( "Are you ready?", 100, 100, snake_font, white)
    Font.Draw ("Yaaa!", 800, 95, snake_font, green)
    loop
	Input.KeyDown(chars)
	mousewhere (mx, my, mb)
	locate (1, 1)
	...
	if mb = 1 and mx > 770 and mx < 980 and my > 70 and my < 150 or chars ('y')then
	    mode := "continue"
	    mouse_button_up
	    cls
	    drawfillbox (0, 0, maxx, maxy, 7)
...
function difficulty : string
    ...
    View.Update
    snake_fontnew := Font.New ("Impact:50")
    Font.Draw ("Choose a difficulty:", 50, 500, snake_fontnew, black)
    Font.Draw ("NOOB", 100, 400, snake_fontnew, green)
    Font.Draw ("AVERAGE", 270, 300, snake_fontnew, yellow)
    Font.Draw ("EXPERT", 500, 200, snake_fontnew, 41)
    Font.Draw ("IMPOSSIBLE", 700, 100, snake_fontnew, red)
    loop
	...
	Input.KeyDown(chars)
	if mb = 1 and mx > 80 and mx < 260 and my > 390 and my < 450 or chars ('n')then
	    level := "noob"
	    cls
	    drawfillbox (0, 0, maxx, maxy, 7)
	    exit
	elsif mb = 1 and mx > 250 and mx < 520 and my > 300 and my < 360 or chars ('a')then
	    level := "average"
	    cls
	    drawfillbox (0, 0, maxx, maxy, 7)
	    exit
	elsif mb = 1 and mx > 490 and mx < 690 and my > 200 and my < 265 or chars ('e')then
	    level := "expert"
	    cls
	    drawfillbox (0, 0, maxx, maxy, 7)
	    exit
	elsif mb = 1 and mx > 690 and mx < 1020 and my > 100 and my < 165 or chars ('i')then
	    level := "impossible"
	    cls
	    drawfillbox (0, 0, maxx, maxy, 7)
	    exit
...
function drawsnake (mode, level : string) : int
    ...
    game_over_font := Font.New ("impect:50")
    ...
    if mode = "continue" then
		...
		loop
		    ...
		    if sqrt ((enemyx - x2) ** 2 + (enemyy - y2) ** 2) < 9  then     %checks if it eats ball
		drawfilloval (enemyx, enemyy, 5, 5, 7)
		ball_count += 2
		enemy_count += 2
		score += points     %adds to score
		ball := "not drawn"   % makes ball not drawn so it know to draw a new one
	    ...
	    if x2 < 0 or x2 > maxx or y2 < 0 or y2 > maxy or direction2 = "up" and whatdotcolour (x2, y2 + 6) = 10 
	    or direction2 = "up" and whatdotcolour (x2, y2 + 6) = 0 or direction2 = "right" and whatdotcolour (x2 + 6, y2) = 10 
	    or direction2 = "right" and whatdotcolour (x2 + 6, y2) = 0 or direction2 = "left" and whatdotcolour (x2 - 6, y2) = 10 
	    or direction2 = "left" and whatdotcolour (x2 - 6, y2) = 0 or direction2 = "down" and whatdotcolour (x2, y2 - 6) = 10 
	    or direction2 = "down" and whatdotcolour (x2, y2 - 6) = 0 then     
		...
		snake1win := Pic.FileNew ("player1win.jpg")
		%Pic.Draw (snake1win, 0, 0, picCopy)
		View.Update
		Font.Draw ("Player 1 wins!", midx-250, midy, game_over_font, 0)
		delay(2500)
		cls
		drawfillbox (1, 1, maxx, maxy, 7)
		exit
		...
		drawfillbox (1, 1, maxx, maxy, 7)
		snake2win := Pic.FileNew ("player2win.jpg")
		% Pic.Draw (snake2win, 0, 0, picCopy)
		 View.Update
		Font.Draw ("Player 2 wins!", midx-250, midy, game_over_font, 0)
		delay(2500)
		cls
		drawfillbox (1, 1, maxx, maxy, 7)
		exit 
...
function game_over (score : int, mode : string) : string
    ...
    View.Update
    game_over_font := Font.New ("serif:15")         %sets fonts
    lose_font := Font.New ("impact:50")
    play_again_font := Font.New ("impact:40")
    Font.Draw (" Game   Over", midx - 200, midy, lose_font, white)
    delay (1500)
	cls
	drawfillbox (1, 1, maxx, maxy, 7)
	
    againcover := Pic.FileNew ("background3.jpg")
    Pic.Draw (againcover, 0, 0, picCopy)
    View.Update
    Font.Draw ("Wanna play again?", 250, 350, play_again_font, 0)
    Font.Draw ("Please press 'y' or 'n' ", 250, 250, play_again_font, 0)
    loop
	Input.KeyDown(chars)
	mousewhere (mx, my, mb)
	 if chars ('y')then
...
loop
	...
	cls
	drawfillbox (1, 1, maxx, maxy, 7)       
	Font.Draw ("Turn it off then...", 700, 50, game_over_font, 0)     
	exit
    end if
end loop
